...
const OrderList = () => {
 ...
 className={`px-4 py-2 rounded-lg text-sm font-medium capitalize border ${filter === option
 ...
 <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary-color w-5 h-5"/>
 <input
 type="text"
 placeholder="Search by customer name, order ID, or product..."
 value={searchQuery}
 onChange={handleSearchChange}
 className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-color/20 focus:border-primary-color shadow-sm"
 />
 </div>
 ...
 <div key={order.id} className="bg-white border border-slate-100 rounded-xl p-4 shadow-sm hover:shadow-md border-l-4 border-l-primary-color group">
 ...
 <Link href={`order-details/${order.id}`} className="flex items-center gap-1 px-5 py-2 rounded-full border border-btn-color/20 bg-btn-color/5 text-btn-color text-sm font-bold hover:bg-btn-color hover:text-white transform">
 ...
 <div className="bg-secondary/10 px-3 py-1 rounded-lg border border-secondary/20 inline-flex items-center gap-1.5 transform">
 ...
 <button className="lg:hidden w-full py-3 rounded-xl bg-slate-50 text-slate-600 font-bold flex items-center justify-center gap-2 hover:bg-slate-100 mt-2">
 ...
 className={`w-10 h-10 rounded-lg text-sm font-bold border ${currentPage === num
