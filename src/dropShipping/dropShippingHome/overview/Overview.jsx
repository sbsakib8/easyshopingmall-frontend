      ...
      <Container className="relative z-10">
        ...
        <div className="container grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {overview.map((view) => (
            <Link
              key={view.id}
              className={`py-12 sm:py-16 md:py-20 px-4 sm:px-8 md:px-10 cursor-pointer rounded-3xl relative overflow-hidden flex flex-col items-center justify-center ${view.class}`}
              href={view.href}
            >
              <div>{view?.icon}</div>
              <h2 className="text-center text-lg sm:text-xl font-bold relative z-10">
                {" "}
                {view.text}
              </h2>
            </Link>
