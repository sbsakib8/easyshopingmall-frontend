...
export const SubCategoryCardSectionLoading = () => {
  return (
        ...
        {[...Array(2)].map((_, groupIndex) => (
          <div key={groupIndex}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-6">
              {[...Array(6)].map((_, idx) => (
                <SubCategoryCardSkeleton key={idx} idx={idx} />
              ))}
            </div>
          </div>
        ))}
...
const SubCategoryCard = ({ image, name, path, idx = 0 }) => {
...
const SubCategoryCardSection = ({ pageType }) => {
  ...
  if (loading) {
    return <SubCategoryCardSectionLoading />;
  }

  // Group subcategories by parent category name dynamically
  const groupedCategories =
    subcategory?.reduce((acc, cat) => {
      ...
      acc[categoryName].push({
        ...
        name: cat.name,
        image: cat.image,
        path: `/sub-category/${cat._id}${pageType ? `?pageType=${pageType}` : ""}`,
      });
...
export default SubCategoryCardSection;
