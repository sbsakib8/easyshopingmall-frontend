...
const DropshippingVideo = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [accessRes, videosRes] = await Promise.all([
        axios.get(`${UrlBackend}/api/video-access/my-access`, {
  ...
  const handleSubmitRequest = async (e) => {
    ...
    try {
      const res = await axios.post(
        `${UrlBackend}/api/video-access/create`,
        {
          ...paymentData,
  ...
  const premiumStatus =
    accessRequests.find((req) => req.videoType === "premium_training")
      ?.status || "none";
  ...
  const VideoCard = ({ video }) => {
    const isLocked =
      video.videoType === "premium" && premiumStatus !== "approved";
      ...
      <div className="bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm hover:shadow-xl transition-all group overflow-hidden relative h-full flex flex-col">
        <div className="flex items-start justify-between mb-4">
          ...
          {video.videoType === "premium" && (
            <div
              className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 ${premiumStatus === "approved"
  ...
  // Group videos by row
  const row1 = videos
    .filter((v) => v.videoType === "standard" || v.videoType === "demo")
    .slice(0, 2);
  const row2 = videos
    .filter((v) => v.videoType === "free" || v.videoType === "premium")
    .slice(0, 2);
  const otherVideos = videos.filter(
    (v) => ![...row1, ...row2].some((rv) => rv._id === v._id),
  );

  return (
    <section className="min-h-screen bg-slate-50/30 py-8 lg:py-12">
      <Container>
        <div className="mb-12 flex flex-col-reverse md:flex-row md:items-center justify-between gap-3">
        ...
        <div className="space-y-8">
          ...
          {otherVideos.length > 0 && (
            <div className="mt-16">
              <h3 className="text-xl font-black text-gray-900 uppercase tracking-widest mb-6 ml-2">
                Previously Shown & Extra Content
              </h3>
          ...
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl relative">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400 text-sm font-bold mt-1">
                Get lifetime access to elite training
              </p>
            </div>
            ...
            <div className="bg-emerald-50/50 p-4 rounded-3xl border border-emerald-100 mb-8">
              ...
              <p className="text-xs text-emerald-700 font-medium leading-relaxed">
                Please send <span className="font-black">৳500</span> (Send
                Money) to our Bkash/Nagad number. Then provide the transaction
                details below.
              </p>
            </div>
