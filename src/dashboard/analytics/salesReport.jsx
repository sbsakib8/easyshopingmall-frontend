...
const SalesReportDashboard = () => {
  ...
  const filteredData = useMemo(() => {
    return allOrders?.filter((item) => {
      ...
      const statusMatch =
        filterStatus === "all" || item.order_status === filterStatus;
      return dateMatch && statusMatch;
    });
  }, [dateRange, filterStatus, searchTerm, allOrders]);
  let totalPage = Math.ceil((filteredData?.length || 0) / skip);
  // Search data
  ...
  const stats = useMemo(() => {
    ...
    const conversionRate =
      totalOrders > 0 ? (completed.length / totalOrders) * 100 : 0;
  ...
  return (
    <section className="min-h-dvh bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-8 overflow-hidden">
      <div className="max-w-[1600px] mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-400">
              Track your sales, orders, and performance in real-time
            </p>
          </div>
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg flex items-center gap-2 ${
                chartType === "line"
                  ? "bg-gradient-to-r from-cyan-500 to-blue-500 text-slate-300 shadow-lg shadow-cyan-500/50"
          ...
          {statsData.map((stat, index) => {
            ...
            return (
              <div
                key={index}
                className={cn(
                  "bg-gradient-to-br rounded-2xl shadow-2xl border border-white/10 p-6 relative overflow-hidden group",
                  `from-${stat.gradient.split(" ")[0]} to-${stat.gradient.split(" ")[1]}`,
                )}
              >
                ...
                <div className="relative">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className={cn("p-2 rounded-lg", stat.iconBg)}>
                      <Icon className={cn(stat.iconColor)} size={20} />
                    </div>
                  </div>
                  ...
                  <p className={cn("text-sm", stat.changeColor)}>
                    {stat.change}
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="relative z-[9999]">
              <button
                ...
                className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-gray-300 px-4 py-2 rounded-lg"
              >
                <Calendar size={18} />
            ...
            <select
              ...
              className="bg-slate-800 hover:bg-slate-700 text-gray-300 px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500"
            >
            ...
            <button className="ml-auto bg-gradient-to-r from-cyan-500 to-blue-500 hover:from-cyan-600 hover:to-blue-600 text-slate-300 px-6 py-2 rounded-lg flex items-center gap-2 shadow-lg shadow-cyan-500/30">
              <Download size={18} />
              Export Report
            </button>
          </div>
        ...
        <div className="bg-slate-900/50 backdrop-blur-xl rounded-2xl p-6 mb-8 border border-slate-700/50">
          ...
          {loading ? (
            <>
              <div className="w-full h-[400px] bg-[#1e293b] rounded-xl flex flex-col p-6 relative overflow-hidden">
                {/* Fake Chart Header / Legend */}
                <div className="flex items-center justify-between mb-6">
                  <div className="flex gap-6">
                    {["Orders", "Cancelled", "Pending", "Sales"].map(
                      (item, i) => (
                ...
                <div className="flex-1 relative flex items-end justify-between gap-2 px-2">
                  {/* Fake Y Axis Labels */}
                  <div className="absolute -left-1 top-0 bottom-8 w-10 flex flex-col justify-between text-right text-xs text-slate-600">
                    {[1000, 800, 600, 400, 200, 0].map((label, i) => (
                      <div
                        key={i}
                        className="h-3 w-8 bg-slate-700 rounded animate-pulse"
                      />
                    ))}
                  </div>
                  ...
                  {Array.from({ length: 7 }).map((_, i) => (
                    ...
                    >
                      {/* Multiple segments to simulate stacked or multiple lines */}
                      <div
                        className="w-full bg-violet-500/30 rounded-t animate-pulse"
                        style={{ height: `${Math.random() * 60 + 30}%` }}
                      />
                      <div
                        className="w-full bg-red-500/30 rounded-t animate-pulse"
                        style={{ height: `${Math.random() * 40 + 15}%` }}
                      />
                      <div
                        className="w-full bg-orange-500/30 rounded-t animate-pulse"
                        style={{ height: `${Math.random() * 25 + 10}%` }}
                      />
                    </div>
                  ))}
                </div>
                ...
                {/* Fake X Axis */}
                <div className="h-8 flex items-center justify-between mt-4 px-8">
                  {Array.from({ length: 7 }).map((_, i) => (
                    <div
                      key={i}
                      className="h-3 w-10 bg-slate-700 rounded animate-pulse"
                    />
                  ))}
                </div>
                ...
                {loading
                  ? Array.from({ length: 10 }, (_, i) => (
                      ...
                      >
                        ...
                        <td className="py-4 px-4">
                          <div className="h-4 w-12 bg-slate-700 rounded mx-auto" />
                        </td>
                        <td className="py-4 px-4">
                  ...
                  : sortedData
                      ?.slice(skip * currentPage, skip * (currentPage + 1))
                      .map((order) => (
                        <tr
                          key={order?.orderId}
        ...
        <div className="flex justify-center flex-wrap mt-5 gap-y-3">
          {[...Array(totalPage ?? 0)].map((_, i) => (
            <button
              onClick={() => setCurrentPage(i)}
              className={`px-4 py-1 rounded-sm mx-1 ${currentPage == i ? "bg-[#00D3F2]" : "bg-white"}`}
            >
              {i}
            </button>
          ))}
