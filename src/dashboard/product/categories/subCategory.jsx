...
const AddSubcategoriesComponent = () => {
  ...
  const bulkActivate = async () => {
    try {
      setSubcategories((prev) =>
        prev.map((sub) => ({ ...sub, isActive: true })),
      );
  ...
  const bulkDeactivate = async () => {
    try {
      setSubcategories((prev) =>
        prev.map((sub) => ({ ...sub, isActive: false })),
      );
      ...
      <div className="lg:ml-10 lg:px-9">
        ...
        <div className="mb-8 bg-white/10 backdrop-blur-lg rounded-2xl p-6 shadow-2xl border border-white/20">
          <div className="flex flex-col sm:flex-row sm:items-start gap-4 justify-between">
            <div className="flex flex-wrap gap-4 flex-1 w-full">
              ...
              {/* View Mode */}
              <div className="flex bg-white/10 rounded-xl border border-white/20 overflow-hidden max-w-[90px]">
                <button
        ...
        <div className="mt-8 bg-white/10 backdrop-blur-lg rounded-2xl p-6 shadow-2xl border border-white/20">
          ...
          <div className="bg-white/5 rounded-xl p-4 border border-white/10 space-y-6">
                ...
                <div key={category._id} className="mb-4 last:mb-0">
                  ...
                  <div className="ml-6 space-y-2">
                    {categorySubcategories.map((subcat) => (
                      <div
                        key={subcat._id}
                        className="flex items-center justify-between p-3 bg-white/3 rounded-lg hover:bg-white/5"
                      >
            ...
            {categories.every(
              (cat) =>
                subcategories.filter((sub) => sub.parentCategory === cat._id)
                  .length === 0,
            ) && (
