...
const AddProductComponent = () => {
  ...
  const addweight = () => {
    if (
      newWeight.trim() &&
      !formData.productWeight.includes(newWeight.trim())
    ) {
      setFormData((prev) => ({
        ...prev,
  ...
  const removeweight = (weightToRemove) => {
    setFormData((prev) => ({
      ...prev,
      productWeight: prev.productWeight.filter(
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 overflow-hidden">
      {/* main section */}
      <div className={` py-5 lg:ml-15 px-2 lg:px-9`}>
        {/* Header */}
        <div className="mb-8">
              ...
              <div className="flex items-center space-x-2">
                {[1, 2, 3, 4, 5].map((star) => (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setFormData((prev) => ({ ...prev, ratings: star }))
                    }
                    className=""
                  >
                    <Star
