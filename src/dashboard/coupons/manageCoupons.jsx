...
const ManageCoupons = () => {
  ...
  const handleEdit = (coupon) => {
    setFormData({
      ...
      applicableCategory:
        coupon.applicableCategory?._id || coupon.applicableCategory || "",
      applicableSubCategory:
        coupon.applicableSubCategory?._id || coupon.applicableSubCategory || "",
      applicableProduct:
        coupon.applicableProduct?._id || coupon.applicableProduct || "",
      ...
      <Container>
        ...
        <div className="relative bg-gradient-to-r from-gray-900/80 via-indigo-900/80 to-purple-900/80 backdrop-blur-xl rounded-3xl p-4 md:p-6 border border-gray-700/50 shadow-2xl flex flex-col md:flex-row md:items-center justify-between overflow-hidden mb-8 animate-slideDown">
          ...
          <div className="absolute inset-0 pointer-events-none">
            ...
            <div className="absolute bottom-4 right-1/3 w-2 h-2 bg-purple-500/30 rounded-full"></div>
          </div>
          ...
          <div className="relative z-10">
            ...
            <p className="text-gray-300 text-sm md:text-base">
              Create and control discount offers for your customers
            </p>
          </div>

          <button
            onClick={() => setShowForm(!showForm)}
            className="mt-4 md:mt-0 px-6 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white rounded-2xl font-semibold shadow-lg transform items-center space-x-2 border border-white/10 inline-flex text-sm md:text-base max-w-[180px] ml-auto"
          >
        ...
        {/* List Section */}
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-4 py-6 md:p-8 shadow-2xl overflow-hidden min-h-[400px]">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-6">
            <h2 className="text-xl md:text-2xl font-medium text-white flex items-center text-nowrap">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
              {filteredCoupons.map((coupon) => {
                ...
                return (
                  <div
                    key={coupon._id}
                    className={`group relative bg-black/40 border ${coupon.isActive ? "border-white/10" : "border-red-900/50"} rounded-3xl p-6 hover:bg-white/10 hover:shadow-2xl overflow-hidden flex flex-col`}
                  >
                    <div>
                      {/* Coupon Icon/Type */}
                      <div className="flex flex-wrap-reverse items-center gap-4 mb-4">
                        <div className="flex-1 flex items-center gap-4">
                          <div
                            className={cn(
                              "size-9 md:size-10 rounded-2xl flex items-center justify-center",
                              {
                        ...
                        <div className="flex">
                          <div
                            className={`px-2 py-1 rounded-md text-[7px] font-bold uppercase tracking-wider ${
                              isExpired
                      ...
                      <div className="mb-6 space-y-2 py-3 border-t border-b border-white/5">
                        {coupon.applicableCategory && (
                          <div className="flex items-center text-xs text-indigo-300">
                            ...
                            Cat:{" "}
                            {typeof coupon.applicableCategory === "object"
                              ? coupon.applicableCategory.name
                              : categories.find(
                                  (c) =>
                                    c._id ===
                                    (coupon.applicableCategory?._id ||
                                      coupon.applicableCategory),
                                )?.name || "Targeted"}
                          </div>
                        )}
                        {coupon.applicableSubCategory && (
                          <div className="flex items-center text-xs text-pink-300">
                            ...
                            Sub:{" "}
                            {typeof coupon.applicableSubCategory === "object"
                              ? coupon.applicableSubCategory.name
                              : subCategories.find(
                                  (s) =>
                                    s._id ===
                                    (coupon.applicableSubCategory?._id ||
                                      coupon.applicableSubCategory),
                                )?.name || "Targeted"}
                          </div>
                        )}
                    ...
                    <div className="flex gap-3 mt-auto">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center space-x-2 py-2 bg-indigo-500/20 hover:bg-indigo-500/40 text-indigo-400 rounded-xl border border-indigo-500/30"
                      >
                        <Edit3 size={14} />
