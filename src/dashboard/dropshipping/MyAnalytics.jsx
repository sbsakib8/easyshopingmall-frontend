...
const MyAnalytics = () => {
  ...
  const trendData = useMemo(() => {
    ...
    return Object.entries(groups)
      .map(([name, value]) => ({ name, value, rawDate: new Date(name + ', ' + new Date().getFullYear()) }))
  ...
  if (dsLoading && !dsAnalytics) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center gap-4 bg-slate-50/30">
        ...
        <div className="text-center">
          <p className="text-gray-900 font-bold text-lg">Analyzing Your Business</p>
          <p className="text-gray-500 text-sm">Fetching real-time performance data...</p>
        </div>
      </div>
    );
  }
  ...
  if (!dsAnalytics && !dsLoading) {
    return (
      <div className="min-h-[80vh] text-center flex flex-col items-center justify-center space-y-6 bg-slate-50/30">
        ...
        <div className="max-w-md">
          ...
          <p className="text-gray-500 mt-2">Start dropshipping products to see your sales performance and referral network growth here.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50/30 pt-24 pb-20 px-4 md:px-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      {/* Header Section */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div className="space-y-1">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-gray-900 tracking-tight">Performance Analytics</h2>
          <p className="text-gray-500">Track your profit margins and network growth in real-time.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={idx} className={`${kpi.bg} p-6 rounded-[2.5rem] border ${kpi.border} relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300 shadow-sm ${kpi.isPrimary ? 'ring-2 ring-teal-500/20 ring-offset-2' : ''}`}>
            ...
            <div className={`absolute -right-4 -bottom-4 w-24 h-24 rounded-full opacity-10 ${kpi.color.replace('text', 'bg')} blur-2xl group-hover:scale-150 transition-transform duration-500`} />
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-[3rem] shadow-sm border border-gray-100 relative group overflow-hidden">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight">Earnings Trend</h3>
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                ...
                <Tooltip 
                  ...
                  formatter={(value) => [`৳${value.toLocaleString()}`, 'Total Earned']}
                />
                <Area 
                  type="monotone" 
                  dataKey="value" 
        ...
        <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-gray-100 relative group overflow-hidden">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-lg font-black text-gray-900 uppercase tracking-tight text-sm">Status Mix</h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            {pipelineData.map((item, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <span className="text-xs font-black text-gray-400 ml-auto">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-50 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-black text-gray-900 uppercase tracking-tight">Recent Activity</h3>
            </div>
            <span className="text-[10px] font-black text-gray-400 bg-gray-50 px-2.5 py-1 rounded-full uppercase">Last 100 Records</span>
          </div>
          <div className="p-2">
              ...
              <div className="space-y-1">
                  ...
                  <div key={idx} className="flex items-center justify-between p-4 rounded-3xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 group">
                    <div className="flex items-center gap-4">
                      <div className={`p-3 rounded-2xl transition-colors ${
                        tx.type === 'profit' ? 'bg-emerald-50 text-emerald-600' : 'bg-blue-50 text-blue-600'
                      }`}>
                        {tx.type === 'profit' ? <DollarSign className="w-4 h-4" /> : <Users className="w-4 h-4" />}
                      </div>
                      <div>
                        <p className="text-sm font-black text-gray-900">
                          {tx.type === 'profit' ? 'Order Profit' : `Referral Bonus: ${tx.user}`}
                        </p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">#{tx.orderId.slice(-6).toUpperCase()}</span>
                          <span className="text-[10px] text-gray-300">•</span>
                          <span className="text-[10px] font-medium text-gray-400">{new Date(tx.date).toLocaleDateString()}</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className={`text-sm font-black ${
                        tx.status === 'credited' ? 'text-emerald-600' : tx.status === 'lost' ? 'text-red-500' : 'text-amber-500'
                      }`}>
                        {tx.status === 'credited' ? '+' : ''}৳{tx.amount.toLocaleString()}
                      </p>
                      <p className={`text-[10px] font-black uppercase tracking-tighter mt-0.5 ${
                        tx.status === 'credited' ? 'text-emerald-500' : tx.status === 'lost' ? 'text-red-500' : 'text-amber-500'
                      }`}>
                        {tx.status}
                      </p>
                    </div>
                ...
                {dsAnalytics.transactions.length === 0 && (
                  <div className="py-20 text-center space-y-3">
                    ...
                    <p className="text-gray-400 text-xs font-medium italic">No recent transactions recorded</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-8 border-b border-gray-50 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-black text-gray-900 uppercase tracking-tight">Referral Network</h3>
            </div>
            ...
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-white z-10 shadow-sm">
                <tr className="bg-slate-50/50">
                  <th className="px-8 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Partner</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Sales</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-right">Income</th>
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">Joined {new Date(ref.lastOrderDate || Date.now()).toLocaleDateString()}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center">
                      <span className="text-sm font-black text-gray-700 bg-gray-100 px-2 py-0.5 rounded-lg">{ref.orderCount}</span>
                    </td>
                    <td className="px-6 py-5 text-right">
                      <p className="text-sm font-black text-blue-600">৳{ref.bonusEarned.toLocaleString()}</p>
                      {ref.pendingBonus > 0 && <p className="text-[10px] text-amber-500 font-black tracking-tighter">৳{ref.pendingBonus.toLocaleString()} pending</p>}
                    </td>
                    <td className="px-8 py-5 text-center">
                      <div className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                        ref.isActive ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'
                      }`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${ref.isActive ? 'bg-emerald-500' : 'bg-slate-400'}`} />
                  ...
                  <tr>
                    <td colSpan={4} className="px-8 py-20 text-center">
                      <div className="flex flex-col items-center gap-3 opacity-30">
                        ...
                        <p className="text-xs font-black uppercase tracking-widest">No Referral Data</p>
                      </div>
                    </td>
                  </tr>
                )}
