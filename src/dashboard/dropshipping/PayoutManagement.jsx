...
const PayoutManagement = () => {
    ...
    const handleReject = async (requestId) => {
        if (!window.confirm("Are you sure you want to reject this request? The balance will be refunded to the user.")) return;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-800 relative overflow-hidden text-white">
            ...
            <div className="relative z-10 p-4 md:p-6 lg:p-8">
                    ...
                    <div className="relative bg-gradient-to-r from-gray-900/80 via-emerald-900/40 to-teal-900/40 backdrop-blur-xl rounded-3xl p-6 sm:p-8 border border-gray-700/50 shadow-2xl shadow-emerald-500/10 overflow-hidden">
                        <div className="relative z-10">
                            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-white mb-2">
                                Payout <span className="bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent">Management</span>
                            </h1>
                            <p className="text-gray-400 text-sm sm:text-base max-w-xl">
                                Review and fulfill withdrawal requests from dropshippers. Maintain financial transparency and speed.
                            </p>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-6 hover:bg-gray-800/50 transition-all group">
                        <div className="flex justify-between items-start mb-4">
                            <div className="p-3 bg-emerald-500/10 rounded-xl text-emerald-400 group-hover:scale-110 transition-transform">
                                <Wallet size={24} />
                            </div>
                            <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500/50">Total Paid</span>
                        </div>
                        <p className="text-3xl font-black text-white">৳{stats.totalAmount.toLocaleString()}</p>
                        <p className="text-xs text-gray-400 mt-1 font-bold">Successfully fulfilled</p>
                    </div>
                    ...
                    <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-6 hover:bg-gray-800/50 transition-all group">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-amber-500/50">Pending</span>
                        </div>
                        <p className="text-3xl font-black text-white">{stats.pending}</p>
                        <p className="text-xs text-gray-400 mt-1 font-bold">Waiting for action</p>
                    </div>
                    ...
                    <div className="bg-gray-800/30 backdrop-blur-sm border border-gray-700/50 rounded-2xl p-6 hover:bg-gray-800/50 transition-all group">
                        <div className="flex justify-between items-start mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-teal-500/50">Total Requests</span>
                        </div>
                        <p className="text-3xl font-black text-white">{stats.total}</p>
                        <p className="text-xs text-gray-400 mt-1 font-bold">Lifetime processed</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredRequests.map((req) => (
                        <div key={req._id} className="bg-gray-800/30 backdrop-blur-xl border border-gray-700/50 rounded-3xl p-6 hover:bg-gray-800/50 transition-all flex flex-col justify-between group h-full">
                            <div>
                                <div className="flex justify-between items-start mb-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-black text-gray-100 text-sm leading-none">{req.userId?.name}</p>
                                            <p className="text-[10px] font-bold text-gray-500 mt-1 uppercase tracking-tighter">{req.userId?.email}</p>
                                        </div>
                                    </div>
                                    <div className={`px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${statusColors[req.status]}`}>
                                        {req.status}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 mb-6">
                                    <div className="bg-black/20 rounded-2xl p-4 flex justify-between items-center border border-white/5 shadow-inner">
                                        <div>
                                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-0.5">Withdrawal Amount</p>
                                            <p className="text-xl font-black text-emerald-400">৳{req.amount.toLocaleString()}</p>
                                        </div>
                                        <div className="text-right">
                                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-0.5">Method</p>
                                            <p className="text-sm font-black text-gray-300">{req.paymentMethod}</p>
                                        </div>
                                    </div>

                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1 ml-1">Receive No.</p>
                                            <div className="bg-gray-900/50 rounded-xl p-3 flex items-center gap-2 border border-white/5">
                                                <Phone size={14} className="text-gray-400" />
                                                <span className="text-xs font-bold text-gray-300">{req.number}</span>
                                            </div>
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1 ml-1">Date</p>
                                            <div className="bg-gray-900/50 rounded-xl p-3 flex items-center gap-2 border border-white/5">
                                                <Clock size={14} className="text-gray-400" />
                                                <span className="text-xs font-bold text-gray-300">{new Date(req.createdAt).toLocaleDateString()}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {req.status === 'approved' && (
                                        <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-4 space-y-3">
                                            <div className="flex items-center justify-between text-[10px] font-black text-emerald-400 uppercase tracking-widest">
                                                ...
                                                <span className="flex items-center gap-1"><Hash size={10} /> {req.transactionId}</span>
                                            </div>
                                            {req.senderNumber && (
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="max-w-md w-full bg-gray-900 border border-gray-800 rounded-[2.5rem] overflow-hidden shadow-2xl p-6 md:p-8 space-y-6">
                        <div className="flex justify-between items-center border-b border-gray-800 pb-4">
                            <div>
                                <h3 className="text-xl font-black text-white tracking-tight">Complete Payout</h3>
                                <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Enter transaction details</p>
                            </div>
                            <button onClick={() => setApprovingId(null)} className="p-2 hover:bg-gray-800 rounded-full transition-colors text-gray-400"><X size={20} /></button>
                        </div>
                        ...
                        {requests.find(r => r._id === approvingId) && (
                            <div className="bg-emerald-500/5 rounded-3xl p-5 space-y-3 border border-emerald-500/10 shadow-inner">
                                <div className="flex justify-between items-center">
                                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Send Amount</span>
                                    <span className="text-2xl font-black text-emerald-400">৳{requests.find(r => r._id === approvingId).amount.toLocaleString()}</span>
                                </div>
                                <div className="h-px bg-gray-800/50 w-full"></div>
                                <div className="flex justify-between items-center">
                                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Method & No.</span>
                                    <div className="text-right">
                                        <p className="text-sm font-black text-gray-200">{requests.find(r => r._id === approvingId).paymentMethod}</p>
                                        <p className="text-xs font-bold text-emerald-400 select-all cursor-pointer">{requests.find(r => r._id === approvingId).number}</p>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        <form onSubmit={handleApprove} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 ml-1">Your Sender No.</label>
                                ...
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 ml-1">Transaction ID</label>
                                    <div className="relative">
                            ...
                            <div>
                                <label className="block text-[10px] font-black uppercase tracking-widest text-gray-500 mb-2 ml-1">Proof of Payment</label>
                                <div className="relative group">
                                    ...
                                    <div className="w-full h-32 border-2 border-dashed border-gray-700 rounded-3xl flex flex-col items-center justify-center gap-2 group-hover:border-emerald-500/50 group-hover:bg-emerald-500/5 transition-all overflow-hidden">
                                        {screenshotPreview ? (
                                            <img src={screenshotPreview} alt="Preview" className="h-full w-full object-cover p-2 rounded-3xl" />
                                        ) : (
                                            <>
                                                <Upload className="w-8 h-8 text-gray-600 group-hover:text-emerald-500" />
                                                <p className="text-[10px] font-black text-gray-500">Drop receipt or click</p>
                                            </>
                                        )}
                                    </div>
                                </div>
            ...
            {selectedImage && (
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="relative max-w-4xl w-full bg-gray-900 border border-gray-800 rounded-[3rem] overflow-hidden p-2 shadow-2xl">
                        ...
                        <img src={selectedImage} alt="Receipt" className="w-full h-auto max-h-[80vh] object-contain rounded-[2.5rem]" />
                        <div className="p-6 flex justify-between items-center bg-gray-900">
                            <div>
                                <p className="text-xs font-black text-gray-100 uppercase tracking-widest">Official Receipt</p>
                                <p className="text-[10px] font-bold text-gray-500">Verified by EasyShoppingMall Admin</p>
                            </div>
                            <a href={selectedImage} target="_blank" rel="noreferrer" className="bg-emerald-500/10 text-emerald-400 px-6 py-3 rounded-xl text-xs font-black hover:bg-emerald-500 hover:text-white transition-all flex items-center gap-2">
