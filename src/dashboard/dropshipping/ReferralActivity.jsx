...
export default function ReferralActivity() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const referralOrders = allOrders.filter(order => order.userId?.referredBy);
        ...
        <div className="bg-gradient-to-r from-gray-800 to-gray-900 rounded-2xl shadow-2xl p-6 md:p-8 mb-8 border border-gray-700">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-white tracking-tight">Referred Customers</h1>
                <p className="text-gray-400 mt-1">Users registered via referral with at least one completed purchase</p>
              </div>
            </div>
            <div className="bg-gray-800/50 px-6 py-3 rounded-2xl border border-gray-700 flex items-center gap-4">
              <div className="text-right">
                <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">Active Referrals</p>
                <p className="text-2xl font-black text-white">{referredUsersData.length}</p>
              </div>
              <button 
                onClick={fetchOrders}
        ...
        <div className="bg-gray-900/40 backdrop-blur-2xl rounded-3xl border border-gray-700/50 overflow-hidden shadow-2xl">
          {loading ? (
            <div className="py-32 flex flex-col items-center gap-4">
              ...
              <p className="text-gray-400 font-medium animate-pulse">Analyzing referral network...</p>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-black/40 border-b border-gray-800">
                    <th className="px-8 py-5 text-xs font-black text-gray-500 uppercase tracking-widest">Customer Information</th>
                    <th className="px-8 py-5 text-xs font-black text-gray-500 uppercase tracking-widest">Referrer</th>
                    <th className="px-8 py-5 text-xs font-black text-gray-500 uppercase tracking-widest text-center">Orders</th>
                    <th className="px-8 py-5 text-xs font-black text-gray-500 uppercase tracking-widest">Total Spent</th>
                    <th className="px-8 py-5 text-xs font-black text-gray-500 uppercase tracking-widest">Last Activity</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800/50">
                  {paginatedData.map((data) => (
                    <tr key={data.customer._id} className="hover:bg-white/[0.02] transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-base font-bold text-white group-hover:text-emerald-400 transition-colors">{data.customer.name}</p>
                            <p className="text-xs text-gray-500 font-medium">{data.customer.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                          <span className="text-sm font-bold text-gray-300">{data.referrer?.name || 'System'}</span>
                          <span className="text-[10px] text-gray-500 bg-gray-800 px-2 py-0.5 rounded-md w-fit mt-1">Code: {data.referrer?.referralCode}</span>
                        </div>
                      </td>
                      <td className="px-8 py-6 text-center">
                        <div className="inline-flex flex-col items-center">
                          <span className="text-lg font-black text-white">{data.completedCount}</span>
                          <span className="text-[10px] text-gray-500 uppercase font-black tracking-tighter">Delivered</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                          <span className="text-lg font-black text-emerald-400">৳{data.totalSpent.toLocaleString()}</span>
                          <span className="text-[10px] text-gray-500 font-bold uppercase">LTV</span>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-gray-300">{new Date(data.lastOrderDate).toLocaleDateString()}</span>
                          <span className="text-[10px] text-gray-500 italic">Latest Order</span>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            <div className="py-32 flex flex-col items-center gap-4 text-center">
              ...
              <h3 className="text-2xl font-bold text-gray-300">No customers found</h3>
              <p className="text-gray-500 max-w-md mx-auto">We couldn't find any customers who registered via referral and have a completed order.</p>
            </div>
          )}
        </div>
        ...
        {totalPages > 1 && (
          <div className="mt-8 flex flex-col sm:flex-row items-center justify-between gap-6 bg-gray-800/20 p-6 rounded-3xl border border-gray-700/30">
            <p className="text-sm text-gray-500 font-medium">
              Showing <span className="text-white font-bold">{((currentPage - 1) * itemsPerPage) + 1}</span> to <span className="text-white font-bold">{Math.min(currentPage * itemsPerPage, referredUsersData.length)}</span> of <span className="text-white font-bold">{referredUsersData.length}</span> customers
            </p>
                  ...
                  .map((page, index, array) => {
                    ...
                    return (
                      <React.Fragment key={page}>
                        {showEllipsis && <span className="text-gray-600 self-end mb-3">...</span>}
                        <button
                          onClick={() => {
                            setCurrentPage(page);
                            window.scrollTo({ top: 0, behavior: 'smooth' });
